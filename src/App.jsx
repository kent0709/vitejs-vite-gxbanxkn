...
import { Sparkles, Play, ChevronLeft, ChevronRight, Bookmark, BookmarkCheck, Library, GraduationCap, Loader2, RotateCcw, Type, Puzzle, Shuffle, Layers, RefreshCw } from 'lucide-react';
...
const HANGUL_BASIC = {
  consonants: [
    { ko: "ㄱ", pro: "g/k", tts: "가", hook: "象形：舌根頂住軟顎。聯想：槍(Gun)。" },
    { ko: "ㄴ", pro: "n", tts: "나", hook: "象形：舌尖頂住上齒齦。聯想：鼻子(Nose)。" },
    { ko: "ㄷ", pro: "d/t", tts: "다", hook: "象形：ㄴ加一橫。聯想：門(Door)。" },
    { ko: "ㄹ", pro: "r/l", tts: "라", hook: "象形：舌頭彈動。聯想：蛇(Reptile)爬行。" },
    { ko: "ㅁ", pro: "m", tts: "마", hook: "象形：雙唇。聯想：嘴巴(Mouth)的方形。" },
    { ko: "ㅂ", pro: "b/p", tts: "바", hook: "象形：ㅁ加兩豎。聯想：水桶(Bucket)。" },
    { ko: "ㅅ", pro: "s", tts: "사", hook: "象形：牙齒。聯想：站立(Stand)的雙腿。" },
    { ko: "ㅇ", pro: "ng", tts: "아", hook: "象形：喉嚨圓形。字首不發音，字尾發ng。" },
    { ko: "ㅈ", pro: "j/ch", tts: "자", hook: "在ㅅ加一橫。聯想：果汁(Juice)杯插吸管。" },
    { ko: "ㅊ", pro: "ch'", tts: "차", hook: "激音：ㅈ加一點。聯想：火車(Choo-choo)。" },
    { ko: "ㅋ", pro: "k'", tts: "카", hook: "激音：ㄱ加一橫。聯想：鑰匙(Key)。" },
    { ko: "ㅌ", pro: "t'", tts: "타", hook: "激音：ㄷ加一橫。聯想：英文字母E，發T音。" },
    { ko: "ㅍ", pro: "p'", tts: "파", hook: "激音：ㅂ變形。聯想：神廟羅馬柱(Pillar)。" },
    { ko: "ㅎ", pro: "h", tts: "하", hook: "象形：喉嚨帶氣流。聯想：戴帽子的人頭(Head)。" }
  ],
  vowels: [
    { ko: "ㅏ", pro: "a", name: "아", hook: "人(|)在地上，太陽(.)東方升起。明亮音 a。" },
    { ko: "ㅑ", pro: "ya", name: "야", hook: "兩個太陽(短橫)，發音前加上 y 變 ya。" },
    { ko: "ㅓ", pro: "eo", name: "어", hook: "人(|)，太陽西方落下。陰暗音 eo(像ㄜ)。" },
    { ko: "ㅕ", pro: "yeo", name: "여", hook: "兩個太陽，加上 y 變 yeo。" },
    { ko: "ㅗ", pro: "o", name: "오", hook: "大地(-)，太陽在上面。明亮音 o。" },
    { ko: "ㅛ", pro: "yo", name: "요", hook: "兩個太陽，加上 y 變 yo。" },
    { ko: "ㅜ", pro: "u", name: "우", hook: "大地(-)，太陽在下面。陰暗音 u(像嗚)。" },
    { ko: "ㅠ", pro: "yu", name: "유", hook: "兩個太陽，加上 y 變 yu。" },
    { ko: "ㅡ", pro: "eu", name: "으", hook: "象形：平坦大地。嘴扁平向側拉開發 eu。" },
...
// 您的專屬 Firebase 設定 (上線到 Vercel 時會自動啟用這組)
...
// 如果在 Canvas 裡，用 Canvas 預設環境；如果不在，就用您專屬的 myFirebaseConfig
...
export default function App() {
  // ==========================================
  // 🚨 緊急修復：動態注入 Tailwind CSS 魔法 🚨
  // ==========================================
  useEffect(() => {
    // 當這個 App 啟動時，檢查有沒有穿上 Tailwind 的衣服，沒有的話自動穿上！
  ...
  const [words, setWords] = useState([
    {
      ...
      memoryHook: "漢字詞「歡迎(환영)」。可以聯想成：換(hwan)個心情來迎接(yeong)美好的事物。",
      imagePrompt: "A warm and welcoming party with balloons and confetti, aesthetic illustration"
    }
  ]);
  ...
  const generateWords = async () => {
    ...
    try {
      // 🛡️ 資安升級：改為呼叫我們自己的 Vercel 後端 API
  ...
  const toggleSaveWord = async (word, e) => {
    if (e) e.stopPropagation();
    if (!user || !db) {
      setAuthError("⚠️ 無法儲存：請先在 Firebase Console 中啟用「匿名登入」功能！");
      return;
    }
        ...
        {view === 'learn' && (
          <div className="flex flex-col items-center">
            <div className="w-full max-w-lg bg-white p-4 rounded-[1.5rem] shadow-sm border border-slate-200 mb-8 flex gap-3">
              <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="輸入想學的主題 (如：咖啡廳點餐)" className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-lg text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium" onKeyDown={(e) => e.key === 'Enter' && generateWords()} />
              <button onClick={generateWords} disabled={loading} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-70 flex items-center gap-2 whitespace-nowrap">
            ...
            {words.length > 0 && (
              <div className="w-full max-w-lg perspective-1000">
                <div className="relative w-full h-[75vh] max-h-[45rem] min-h-[36rem] transition-transform duration-500 ease-out cursor-pointer" style={{ transformStyle: 'preserve-3d', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }} onClick={() => setIsFlipped(!isFlipped)}>
                  ...
                  <div className="absolute inset-0 bg-white shadow-2xl rounded-[2.5rem] flex flex-col overflow-hidden border-2 border-slate-100" style={{ backfaceVisibility: 'hidden' }}>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 relative">
                      <h2 className="text-[5rem] md:text-[6rem] font-black text-slate-800 mb-4 tracking-tighter text-center leading-tight break-words w-full">{currentWord.ko}</h2>
        ...
        {view === 'alphabet' && (
          <div className="max-w-4xl mx-auto pb-10">
            <div className="mb-10 text-center">
              ...
              <p className="text-slate-500 text-lg leading-relaxed font-medium">先認識核心字母，再利用巨大化的工具進行拼字訓練！</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
              <div>
                <h3 className="text-2xl font-black text-indigo-700 mb-6 flex items-center gap-3 border-b-4 border-indigo-100 pb-3">
                  <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-xl text-lg">14個</span> 基本子音 <span className="text-base text-indigo-400 font-medium ml-auto">※ 搭配「ㅏ(a)」</span>
                </h3>
                <div className="grid grid-cols-4 gap-3">
                  {HANGUL_BASIC.consonants.map(item => (
                    <div key={item.ko} onClick={() => { setActiveLetter(activeLetter === item.ko ? null : item.ko); speak(item.tts); }} className={`relative cursor-pointer h-24 rounded-[1.25rem] border-2 transition-all duration-300 overflow-hidden ${activeLetter === item.ko ? 'border-indigo-500 shadow-lg scale-105' : 'border-slate-200 bg-white hover:border-indigo-400'}`}>
                      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-opacity ${activeLetter === item.ko ? 'opacity-0' : 'opacity-100'}`}>
                        <span className="text-4xl font-black text-slate-800">{item.ko}</span>
                      </div>
                      <div className={`absolute inset-0 bg-indigo-600 text-white p-2 flex flex-col justify-center transition-opacity ${activeLetter === item.ko ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                        <span className="text-center font-black text-xl mb-1">{item.ko}</span>
                        <p className="text-xs leading-tight opacity-90 text-center font-medium">{item.hook.replace('象形：', '').split('。')[0]}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <h3 className="text-2xl font-black text-amber-600 mb-6 flex items-center gap-3 border-b-4 border-amber-100 pb-3">
                  <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-xl text-lg">10個</span> 基本母音
                </h3>
                <div className="grid grid-cols-4 gap-3">
                  {HANGUL_BASIC.vowels.map(item => (
                    <div key={item.ko} onClick={() => { setActiveLetter(activeLetter === item.ko ? null : item.ko); speak(item.name); }} className={`relative cursor-pointer h-24 rounded-[1.25rem] border-2 transition-all duration-300 overflow-hidden ${activeLetter === item.ko ? 'border-amber-500 shadow-lg scale-105' : 'border-slate-200 bg-white hover:border-amber-400'}`}>
                      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-opacity ${activeLetter === item.ko ? 'opacity-0' : 'opacity-100'}`}>
                        <span className="text-4xl font-black text-slate-800">{item.ko}</span>
                      </div>
                      <div className={`absolute inset-0 bg-amber-500 text-white p-2 flex flex-col justify-center transition-opacity ${activeLetter === item.ko ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                        <span className="text-center font-black text-xl mb-1">{item.ko}</span>
                        <p className="text-xs leading-tight opacity-90 text-center font-medium">{item.hook.split('。')[0]}</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="w-full max-w-md perspective-1000 mb-10">
                    <div className="relative w-full h-[28rem] min-h-[400px] transition-transform duration-500 ease-out cursor-pointer" style={{ transformStyle: 'preserve-3d', transform: comboFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }} onClick={() => setComboFlipped(!comboFlipped)}>
                      ...
                      <div className="absolute inset-0 bg-white shadow-2xl rounded-[3rem] flex flex-col items-center justify-center p-8 border-2 border-indigo-50" style={{ backfaceVisibility: 'hidden' }}>
                        ...
                        <h2 className="text-[9rem] leading-none font-black text-slate-800 mb-6 drop-shadow-md">{currentCombo.ko}</h2>
                        <span className="text-4xl font-mono text-slate-500 mb-8 bg-slate-100 px-8 py-2 rounded-full font-bold">{currentCombo.pro}</span>
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-indigo-50 to-blue-50 shadow-2xl rounded-[3rem] flex flex-col p-10 border-2 border-indigo-100 overflow-y-auto" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
                        <div className="flex-1 flex flex-col items-center justify-center text-center">
                          ...
                          <div className="flex items-center justify-center gap-8 mb-10 bg-white px-10 py-6 rounded-[2rem] shadow-md border border-indigo-50 w-full">
                            ...
                            <span className="text-6xl font-bold text-indigo-600 font-mono">{currentCombo.pro}</span>
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center">
                    {selectedC && selectedV && (
                      <div className="flex flex-col items-center bg-white p-10 rounded-[3rem] shadow-xl border-4 border-indigo-50 w-full">
                        ...
                        <span className="text-4xl font-mono text-slate-500 mb-8 bg-slate-100 px-8 py-2 rounded-full font-bold">{selectedC.pro.split('/')[0]}{selectedV.pro}</span>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap justify-center gap-3">
                        {HANGUL_BASIC.consonants.map(c => (
                          <button key={c.ko} onClick={() => setSelectedC(c)} className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl font-black text-2xl transition-all ${selectedC?.ko === c.ko ? 'bg-indigo-600 text-white shadow-lg scale-110' : 'bg-white text-slate-600 border-2 border-slate-200 hover:border-indigo-400 hover:text-indigo-600'}`}>
                            {c.ko}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap justify-center gap-3">
                        {HANGUL_BASIC.vowels.map(v => (
                          <button key={v.ko} onClick={() => setSelectedV(v)} className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl font-black text-2xl transition-all ${selectedV?.ko === v.ko ? 'bg-amber-500 text-white shadow-lg scale-110' : 'bg-white text-slate-600 border-2 border-slate-200 hover:border-amber-400 hover:text-amber-600'}`}>
                            {v.ko}
                          </button>
                        ))}
                      </div>
